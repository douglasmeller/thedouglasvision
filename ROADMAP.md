...
- **Notícias (feito)**: resumo diário por IA dos 4 sites de interesse (Flow Games, Meu Timão, Reforma Tributária, Ei Nerd), gerado via Edge Function + cron diário ou botão "Atualizar agora". Resumo dividido por fonte, 8 manchetes por site (pedido de 22/09; era 6, e 5 antes disso). Quando o site não postou nada nas últimas 24h, o resumo cai num fallback com só as 2 últimas manchetes (não mudou com esse pedido — é só pra não ficar vazio num dia parado).
