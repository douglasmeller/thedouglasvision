...
1. **Jarvis — voz ao vivo e resposta travada (REABERTO)** — ver seção "Jarvis — bugs reportados". É a função central do app e está quebrada de novo.
2. **Inspeção total com o Opus 5** — varredura do app inteiro atrás de bug, com o modelo mais forte. Faz sentido rodar ANTES das features grandes, pra não construir em cima de coisa quebrada.
...
5. **Anotações mais completas** — ver "Novos setores → Anotações".
...
- **Anotações (feito)**: bloco de notas estilo dontpad — formatação rica, autosave.
  ...
  - **Deixar mais completa (pedido de 17/09)**: pedido em aberto, sem lista fechada ainda — alinhar com o Douglas o que ele sente falta antes de sair construindo. Candidatos óbvios: busca por conteúdo (título e corpo, em todas as pastas), anexar imagem/arquivo, fixar e favoritar notas, histórico de versões, exportar nota (PDF/Markdown), links entre notas, ordenação da lista (recentes/nome/criação), e arrastar no celular (hoje é só mouse).
...
- **Cofre de Senhas** (reforçado em 17/09): novo setor pra guardar todas as senhas, organizável por pastas "e tudo mais". Exigências explícitas do Douglas: tem que ser **BEM seguro**; é **obrigatório digitar a senha do app de novo** pra entrar nessa sessão (segunda barreira — não basta já estar logado no TDV). Nome da página: **"Cofre de Senhas"**.
  - Não desenhar a arquitetura sem alinhar antes — dado sensível de verdade. Direção provável: criptografia no próprio navegador (AES-GCM com chave derivada da senha via PBKDF2/Argon2) ANTES de subir pro Supabase, de forma que o banco só guarde texto cifrado e nem quem tem acesso ao banco consiga ler. Pontos a decidir juntos: se a chave é a senha do app ou uma senha-mestra separada (se for a do app, trocar a senha do app exige re-cifrar o cofre); tempo até o cofre trancar sozinho por inatividade; o que acontece se a senha for esquecida (sem ela, os dados são irrecuperáveis — é o preço de ser seguro de verdade); e se o Jarvis pode ou não ter acesso ao cofre (recomendação: não).
...
- Hipóteses a investigar (não confirmadas): volume baixo + voz sumindo apontam pro controle de áudio (o elemento `<audio>` reaproveitado, ou o eco cancelando/abaixando o próprio áudio do Jarvis); não ouvir direito aponta pro barge-in ainda disparando com o eco do próprio Jarvis sem fone; a bolinha eterna aponta pra outro caminho que deixa `jarvisLoading` preso ou pro stream SSE nunca fechando. Diagnóstico precisa ser feito com o app real rodando (console + rede), não só lendo código — já foram duas rodadas de correção "no escuro".
...
## Inspeção total com o Opus 5 (pedido de 17/09)
- Varredura completa do app com o modelo mais forte, no mesmo formato da auditoria de 03/09 (varreduras paralelas por área, cada achado com arquivo:linha e cenário concreto de falha — nada de palpite de estilo), só que cobrindo também o que entrou depois: Anotações repaginadas + arrastar nota→pasta, Agenda com arrastar e tarefas no calendário, hover neon, larguras novas, barra de seleção.
- Incluir as Edge Functions do Supabase (`jarvis-chat`, `news-digest`, `stock-quotes`) e as políticas de acesso (RLS) das tabelas, que a auditoria anterior não cobriu.
